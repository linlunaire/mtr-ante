...
/** Actual source rewrites and MC 26.2 NBT defaults, without launching a game. */
public final class NbtSourceCompatibilityCheck {
    ...
    public static void main(String[] args) throws Exception {
        ...
        binding.setVariable("rootProject", Map.of("ext", extensions));
        new GroovyShell(binding).evaluate(Files.readString(root.resolve("versions/26.2/data-port.gradle")));
        Closure<?> transform = (Closure<?>) extensions.get("transformAnte26DataSource");
        ...
        for (Map.Entry<String, String> entry : receivers.entrySet()) {
            String source = Files.readString(root.resolve("common/src/main/java/cn/zbx1425/mtrsteamloco/" + entry.getKey() + ".java"));
            String result = transform.call(source).toString();
            require(!source.equals(result), "No migration exercised: " + entry.getKey());
            ...
            require(transform.call(result).equals(result), "Transform is not idempotent: " + entry.getKey());
        ...
        String prefix = "package cn.zbx1425.mtrsteamloco.gui; class CompoundCreatorScreen { ";
        String unrelated = prefix + "void json(JsonObject task) { task.getString(Task.TAG_TYPE); } }";
        require(transform.call(unrelated).equals(unrelated), "Unrelated task getter changed");
        String ambiguous = prefix + "void nbt(CompoundTag tag) { tag.getString(\"key\"); } "
                + "void json(JsonObject tag) { tag.getString(\"key\"); } }";
        require(transform.call(ambiguous).equals(ambiguous), "Ambiguous receiver should be left for explicit review");
        String foreign = "package other; class BlockEyeCandy { void read(CompoundTag compoundTag) { compoundTag.getString(\"prefabId\"); } }";
        require(transform.call(foreign).equals(foreign), "A class outside ANTE was changed");
        String receiver = prefix + "void nbt(CompoundTag tag) { tag.getString(\"key\"); holder.tag.getString(\"key\"); messagePackHelper.getString(\"key\"); } }";
        String receiverResult = transform.call(receiver).toString();
        require(receiverResult.contains("CompoundTagMapper.getString(tag, \"key\")"), "Reviewed receiver was not migrated");
        require(receiverResult.contains("holder.tag.getString(\"key\")"), "Member of an unrelated receiver changed");
        require(receiverResult.contains("messagePackHelper.getString(\"key\")"), "Unrelated getter changed");
